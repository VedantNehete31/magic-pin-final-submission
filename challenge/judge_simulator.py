...
BOT_URL = os.getenv("JUDGE_BOT_URL", "http://localhost:8080")

# Choose your LLM provider: "openai", "anthropic", "gemini", "deepseek", "groq", "ollama", "openrouter"
LLM_PROVIDER = os.getenv("JUDGE_LLM_PROVIDER", "openai")

# Your API key (paste your key here)
LLM_API_KEY = os.getenv("JUDGE_LLM_API_KEY", "")

# Model to use (leave empty for default, or specify like "gpt-4o", "claude-3-5-sonnet-20241022", etc.)
LLM_MODEL = os.getenv("JUDGE_LLM_MODEL", "")

# For Ollama only: local server URL
OLLAMA_URL = os.getenv("JUDGE_OLLAMA_URL", "http://localhost:11434")

# Which test to run by default
TEST_SCENARIO = os.getenv("JUDGE_TEST_SCENARIO", "all")
JUDGE_NOW = os.getenv("JUDGE_NOW", "2026-04-26T12:00:00Z")
...
class BotClient:
    ...
    def tick(self, triggers):
        return self._request("POST", "/v1/tick", 15, {
            "now": JUDGE_NOW, "available_triggers": triggers
        })
